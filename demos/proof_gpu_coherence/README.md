...
The HIP lane's lazy chunk protocol in `bend2/lanes/hip.c` in its first
form, before the write map (a leave made every chunk stale): `gpu_fault`,
`gpu_heap`'s enter and leave, `gpu_fetch`, `gpu_touch`, `gpu_publish`;
modelled, with its safety laws proven in Bend.
...
- Host accesses and fault steps run between turns; an enter finds no
  thread in `gpu_fault` (lanes/hip.c 709-712).
...
`logs/cohmodel` (outside the repo) runs the C's functions of that first
form, one pthread per access, against the model: heap ends inside,
...
## Stage 2: the write map

`Stage2.bend` models the write map the lane ships: the device marks what it
