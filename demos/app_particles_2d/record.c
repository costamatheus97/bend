...
// bump off such a chain (at_wsb: adopted) or not (at_wsl: a lane's own
// free list). NULL: off. Heap reads stay below word at_w0 + at_wlim.
...
__device__ INLINE void at_wnew(u64 h, u64 n) {
  ...
  if (h < at_wbump) {
    ...
    at_wset(bank ? at_wsb : at_wsl, h, n);
  }
}
