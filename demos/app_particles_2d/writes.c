...
// slots (1 adopted, 2 a lane's own, 4 at or past the bump); at_pf, what a
...
// per turn: chunks, cs 0..9, rA, rAd, its chunks, chunks with A adopted,
// own, past; per leave: chunks by at_pf and at_wacc (0, only A F P, L U),
// runs of at_pf & 1, 3, 5, at_wc 1, 2, either: avoided, prefetched,
// unused; ph acc live how pf same; ph at_wsrc, acc & 60
static u64 at_wt[2][17], at_wpf[2][8][3], at_wrun[2][3], at_wfree[2][3][3];
static u64 at_wdl[2][64][2][2][8][2], at_wsd[2][8][2];
static hipDeviceptr_t at_wg[9];
static bool at_won;
...
// their snapshot)
static void wr_enter(u64 n, bool dok) {
  static const char* g[9] = { "at_wa", "at_wf", "at_w0", "at_wn", "at_wb",
    "at_wsb", "at_wsl", "at_wlim", "at_wbump" };
  ...
  if (at_wacc == NULL) {
    ...
    for (u32 i = 0; i < 9; i += 1) {
      ...
      at_won = at_won && hipModuleGetGlobal(&at_wg[i], &len, gpu_lib, g[i])
        == hipSuccess && (i > 7 || (hipMalloc((void**)&at_wd[i], i == 3 ? cb
  ...
  void*       wr[5] = { NULL };  // at_wb, at_wsb, at_wsl, at_wf, at_wa
  for (u32 i = 0; on && i < 5; i += 1) {
    wr[i] = at_wd[i < 3 ? 4 + i : i == 3 ? 2 : p];
  }
  u32 z[5] = { p, 2, 4, 5, 6 };
  at_won = at_won && dok && hipMemcpy(at_dev + n * GPU_CHUNK,
    ...
    && wr_set(8, &at_bump);
  for (u32 i = 0; i < 5; i += 1) {  // at_wa last: it turns the rest on
    at_won = at_won && hipMemset(at_wd[z[i]], 0, bb) == hipSuccess
      && wr_set(i < 3 ? 4 + i : i == 3 ? 1 : 0, &wr[i]);
  }
...
static void wr_leave(void) {
  ...
  for (u64 b = 0; ok && b < m * 2048; b += 1u << 20) {
    struct { u64* cur; char* old; u32* wa; u32* wp; u32* wf; u32* wsb;
      u32* wsl; u32* cs; u64 base; u64 n; } args = { gpu_vram + gpu_lo / 8,
      at_dev, at_wd[tp], at_wd[1 - tp], at_wd[2], at_wd[5], at_wd[6],
      at_wd[3], b, m * 2048 };
  ...
  for (u64 c = 0; c < n; c += 1) {
    ...
    bool w  = !kn || k[0];
    t[0] += w;
    for (u32 j = 0; kn && j < 10; j += 1) {
      t[1 + j] += k[j];
    }
    u32 src = kn ? (k[8] ? 1 : 0) | (k[9] ? 2 : 0)
      | (k[2] > k[8] + k[9] ? 4 : 0) : 0;
    for (u32 j = 0; j < 3; j += 1) {
      t[14 + j] += src >> j & 1;
    }
  ...
  for (u64 i = 0; am && i < at_mw; i += 1) {
    t[13] += __builtin_popcount(at_am[i]);
  }
  at_pfph = at_wcph = tp;
  t[11] += at_rA;
  t[12] += at_rAd;
...
static void wr_fault(u64 c, u32 how, u32 r) {
  ...
  at_wsd[ph][at_wsrc[c] & 7][(a & 60) != 0] += 1;
  for (u32 v = 0; v < 3; v += 1) {
